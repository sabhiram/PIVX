...
void ComputePreFactor(std::string strPassphrase, std::string strSalt, uint256& prefactor)
{
    ...
    uint64_t s = uint256S(ReverseEndianString(strSalt)).Get64();
...
bool ComputePasspoint(uint256 passfactor, CPubKey& passpoint)
{
    ...
    int clen = 65;
		// XX42		return secp256k1_ec_pubkey_create(UBEGIN(passpoint), &clen, passfactor.begin(), true) != 0;
		// XX42 (SPOCK) Need to fix
		return 0;
}
...
std::string BIP38_Encrypt(std::string strAddress, std::string strPassphrase, uint256 privKey)
{
    ...
    uint512 hashed;
    uint64_t salt = uint256S(ReverseEndianString(strAddressHash)).Get64();
...
bool BIP38_Decrypt(std::string strPassphrase, std::string strEncryptedKey, uint256& privKey, bool& fCompressed)
{
    ...
    privKey = factorB;
		// XX42 (SPOCK) NEED TO FIX
		// (SPOCK)    if(!secp256k1_ec_privkey_tweak_mul(privKey.begin(), passfactor.begin()))
        return false;
